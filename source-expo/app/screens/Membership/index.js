      ...
      <SafeAreaView style={[BaseStyle.safeAreaView]} edges={['right', 'top', 'left']}>
        ...
        <View style={{ flex: 1 }}>
          <TouchableOpacity style={[styles.card, styles.selected, { backgroundColor: colors.card }]} disabled={true}>
            ...
            {!user.isTrial && !user.isActive && <View style={styles.expireBadge}>
              <Text style={styles.expireText}>{t('expired')}</Text>
            </View>}
          </TouchableOpacity>
        </View>
        <View style={{ margin: 20 }}>
            {!user.isTrial && !user.isActive &&
              <Button full onPress={() => navigation.navigate('Pricing', { isStandByPage: false, isProfilePage: true })} >
                {t('buy_new_package')}
              </Button>
            }
          </View>
