...
import { Header, Icon, ListTextButton, SafeAreaView, Tag, TextInput, Text } from '@/components';
...
export default function PSelectAssignee() {
  ...
  const onSelect = (user) => {
    setFriends(friends.map((item) => (item.id === user.id ? { ...item, isSelect: !user.isSelect } : item)));
  };
